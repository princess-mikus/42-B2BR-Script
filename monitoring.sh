...
printf "LVM Satus: $(if [ $(lsblk | grep lvm | wc -l) -eq 1 ]; then echo 'off'; else echo 'on'; fi)\n"
printf "Number of TCP Connections: $(netstat -an | grep -c 'ESTABLISHED')\n"
printf "Users Online: $(who | wc -l)\n"
printf "IP: $(ip a s | grep 'inet' | grep -v '128.0.0.1' | grep -v 'inet6' | sed 's/\/16//g' | awk '{print $2}') (MAC: $(ip link show | grep link/ether | awk '{print $2}'))\n"
